"""Wire NIOS dnstap messages through the user-managed `Enrichment` pipeline.

Two steps:
  1) Splice `ipam normalize client_ip from dns_client_ip` into stage 1
     of the existing Enrichment pipeline so PTR → client_fqdn fires on
     dnstap events.
  2) Connect the NIOS dnstap stream to the Enrichment pipeline.

Idempotent. Same shape as extend_enrichment_csp.py / _destination.py.
"""
...
RULE_NAME = "ipam normalize client_ip from dns_client_ip"
DNSTAP_STREAM_TITLE = "NIOS dnstap"


def _ensure_rule() -> None:
    spec = json.loads((Path(__file__).parent / "dnstap_qip_normalize.json").read_text())
...
def _splice_into_enrichment() -> None:
    ...
    if resp.get("errors"):
        ...
        sys.exit(2)
    print(f"  Enrichment pipeline updated; stage 1 references {RULE_NAME!r}")


def _connect_dnstap_stream() -> None:
    streams = (gl.api("GET", "streams") or {}).get("streams", [])
    stream = next((s for s in streams if s["title"] == DNSTAP_STREAM_TITLE), None)
    if not stream:
        print(f"  WARN: stream {DNSTAP_STREAM_TITLE!r} not found — skipping connect")
        return
    sid = stream["id"]
    try:
    ...
    if want == have:
        print(f"  stream {DNSTAP_STREAM_TITLE!r} already wired to Enrichment")
        return
    gl.api("POST", "system/pipelines/connections/to_stream", {
        "stream_id": sid,
        "pipeline_ids": sorted(want),
    })
    print(f"  stream {DNSTAP_STREAM_TITLE!r} now connected to Enrichment pipeline")


def main() -> None:
    _ensure_rule()
    _splice_into_enrichment()
    _connect_dnstap_stream()
